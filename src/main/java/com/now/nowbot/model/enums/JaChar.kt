...
enum class JaChar(val hiragana: String, val katakana: String, val romanized: String) {
    ...
    companion object {
        ...
        fun containsJapanese(T: CharSequence): Boolean {
            return T.contains(Regex("[\u3040-\u30ff]"))
        }

        fun isSmallChar(char: Char): Boolean {
            return isEqual(char, A2) || isEqual(char, E2) || isEqual(char, I2) || isEqual(char, O2) || isEqual(char, U2)
        }

        // 促音 さっき -> Sakki
        fun isSokuon(char: Char): Boolean {
            return isEqual(char, TSU2)
        }

        fun isEqual(char: Char, jaChar: JaChar): Boolean {
            return (char.toString() == jaChar.hiragana || char.toString() == jaChar.katakana)
        }
    }
}
