...
public class DiceService implements MessageService<DiceService.DiceParam> {
    ...
    public static String Compare(String s) throws DiceException {
        ...
        // 记得这里才是需要查询的顺序
        final List<Split> splits = Arrays.asList(TIME, TIMES, POSSIBILITY, ACCURACY, AMOUNT, BETTER, COMPARE, OR, WHETHER, AM, WHAT, WHY, WHO, IS, REAL, JUXTAPOSITION, PREFER, HESITATE, EVEN, COULD, RANGE, ASSUME, CONDITION, LIKE, THINK, NEST, QUESTION);

        for (var sp : splits) {
            var onlyC3 =
                    sp == TIME || sp == TIMES || sp == AMOUNT || sp == WHY || sp == WHO ||
            ...
            if (isPerfectMatch(matcher, hasC3, onlyC3)) {
                ...
                switch (split) {
                    ...
                    case TIMES -> {
                        ...
                        double i = getRandomInstantly();

                        if (i > 0.98d) num = 2147483647;
                        else if (i > 0.95d) num = 114514;
        ...
        if (result < boundary - 0.002f) {
            ...
            switch (split) {
                ...
                case TIME, TIMES, POSSIBILITY, ACCURACY -> {
    ...
    private static String ChangeCase(@NonNull String s) {
        ...
        return s.trim()
                ...
                .replaceAll("[习習]近平|[习習]?总书记|主席|国家|政治|反动|反?共(产党)?|[国國]民[党黨]|天安[門门]|极[左右](主义)?|革命|(社会)?主义|自由|解放|中[華华]民[国國]|情趣|迪克|高潮|色[诱情欲色]|擦边|露出|[蛇射受授吞]精|潮喷|成人|性交|小?男娘|小?南梁|做爱|后入|药娘|怀孕|生殖器|寄吧|几把|鸡[鸡巴]|[精卵]子|[精爱]液|子宫|阴[茎蒂唇囊道]|[逼Bb阴吊叼批肛]毛|搞基|出?脚本|[Rr]-?18|18\\s?禁|LGBT", "[和谐]")
