...
@Service("SEARCH") class SearchService(val beatmapApiService: OsuBeatmapApiService) :
    ...
    companion object {
        ...
        private fun constructParam(text: String?): SearchParam? { // Nishigomi Kakumi - Hyakka Ryouran (SugiuraAyano) [Kantan]
            ...
            val matcher = Pattern.compile("([:：](\")?(?<genre>$REG_ANYTHING)(\")?)?\\s*((\")?(?<artist>$REG_ANYTHING)(\")?\\s*-\\s*)?\\s*((\")?(?<title>$REG_ANYTHING)(\")?)?\\s*(\\((\")?(?<creator>$REG_ANYTHING)(\")?\\))?\\s*(\\[(\")?(?<difficulty>$REG_ANYTHING)(\")?])?\\s*(#(\")?(?<status>$REG_ANYTHING)(\")?#?)?\\s*(\\*(\")?(?<sort>$REG_ANYTHING)(\")?\\*?)?\\s*")
                .matcher(str)
        ...
        private fun constructQuery(param: SearchParam): Map<String, Any> {
            val query = mutableMapOf<String, Any>(
                ...
                "sort" to getSort(param.sort),
                "page" to 1
            )

            getStatus(param.status)?.let { query["s"] = it }
            getGenre(param.genre)?.let { query["g"] = it }
            // getLanguage(param.language)?.let { query["l"] = it }
        ...
        private fun getParam(params: Map<String, Any?>): String {
            ...
            return if (map.size == 1 && map.firstNotNullOfOrNull { it.key } == "title") {
