...
class ServiceCountService(
    ...
    private fun getServiceCountMarkdownDaily(
        ...
        sb.append("| 0 | 总计和平均 | ${list.size} | ${avs.values.average().roundToSec()}s | ${mds.values.average().roundToSec()}s | ${mxs.values.average().roundToSec()}s | ${mis.values.average().roundToSec()}s |  | $dauTotal | $stickiness |")
