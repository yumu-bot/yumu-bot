...
@Service("DICE") class DiceService : MessageService<DiceParam>, TencentMessageService<DiceParam> {
    ...
    internal enum class Split(val pattern: Pattern, val onlyC3: Boolean) {
        ...
        BETTER(
            Pattern.compile(
                "\\s*(?<m1>[\\S\\s]*)\\s*(?<c2>(跟|和(?![解气平好局同谐睦])|与(?![法律其国同行作])|并(?![且非发一排列])|\\s(?<![A-Za-z])(and|or|with)(?![A-Za-z])\\s))\\s*(?<m2>[\\S\\s]*?)\\s*(?<![叠面傻装牛菜想提分排开小对阿科此豆死伦攀反字])比?(比[，,\\s]*?哪个|比[，,\\s]*?谁|哪个|谁)更?(?<c3>[\\S\\s]*)"
            ), onlyC3 = false
        ),
