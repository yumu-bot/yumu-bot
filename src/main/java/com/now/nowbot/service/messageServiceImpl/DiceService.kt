...
@Service("DICE") class DiceService : MessageService<DiceParam> {
    ...
    internal enum class Split(val pattern: Pattern, val onlyC3: Boolean) {
        ...
        TIMES(
            Pattern.compile(
                "(?<m1>[\\S\\s]*)?(?<c3>(((?<![一两二三四五六七八九十百千万亿这那上下哪点排报成出提命匿爆真假实大小])(几多?|多少|什么|啥|哪个|何)?(频率|无数)[次个位件名只发层岁人枚字章节]数?)|第几[次个位件名只发层岁人枚字章节]?)(之?[后内])?)(?<m2>[\\S\\s]*)?"
            ), onlyC3 = true
        ), // 次数
        ...
        AMOUNT(
            Pattern.compile(
                "(?<m1>[\\S\\s]*)?(?<c3>[是有]?多少[次个位件名只发层岁人枚字章节]?|数量(?!级)|(?<![一两二三四五六七八九十百千万亿这那上下哪点排报成出提命匿爆真假实大小])[次个位件名只发层岁人枚字章节]数)(?<m2>[\\S\\s]*)?"
            ), onlyC3 = true
        ),
