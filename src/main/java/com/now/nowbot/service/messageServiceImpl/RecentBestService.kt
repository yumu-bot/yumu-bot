    ...
    private fun getParam(event: MessageEvent, matcher: Matcher): RecentBestParam {
        ...
        if (dataID.data != null) {
            ...
            val async = AsyncMethodExecutor.awaitPair(
                ...
                {
                    runCatching {
                        scoreApiService.getBestScores(dataID.data!!, dataMode.data!!, 199, 1)
                    }.getOrNull()?.firstOrNull()
                },
            )

            user = async.first
            minimumPP = max((async.second?.pp ?: (user.pp / 20.0)) * 0.8, 10.0)
            ...
            val last = runCatching {
                scoreApiService.getBestScores(user.userID, dataMode.data!!, 199, 1)
            }.getOrNull()?.firstOrNull()

            minimumPP = max((last?.pp ?: (user.pp / 20.0)) * 0.8, 10.0)
        }
