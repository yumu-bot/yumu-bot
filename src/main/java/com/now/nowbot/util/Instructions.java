...
public class Instructions {
    ...
    public static final Pattern LISTENER = Pattern.compile("^[!！]\\s*(?i)((ym)?make\\s*love|(match)?listen(er)?|(ym)?ml(?![A-Za-z_])|li(?![A-Za-z_]))\\s*(?<matchid>\\d+)?\\s*(?<operate>start|s|stop|p|end|e|off|f|on|o)?");
