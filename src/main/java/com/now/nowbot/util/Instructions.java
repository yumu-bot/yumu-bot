...
public class Instructions {
    ...
    public static final Pattern MAP = Pattern.compile("[!！]\\s*(?i)(ym)?(beatmap|map(?![A-Za-z_])|m(?![A-Za-z_]))\\s*([:：](?<mode>\\w+))?\\s*(?<bid>\\d+)?\\s*(%?(?<accuracy>\\d+\\.?\\d*)%?)?\\s*(x?(?<combo>\\d+\\.?\\d*)x?)?\\s*(\\+(?<mod>( ?[EZNMFHTDRSPCLO]{2})+))?");
