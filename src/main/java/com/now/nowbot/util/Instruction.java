...
public enum Instruction {
    ...
    PPPLUS("ppp", "!ppp [osu name] std的pp+计算", "^[!！]\\s*(?i)ppp(?![vV])(\\s+(?<name>[0-9a-zA-Z\\[\\]\\-_ ]*))?"),
    PPPLUSVS("ppvs", "!ppvs <osu name|@某人> pp+的对比.需要自己绑定,如果是对比@对象也需要被对比人绑定", "^[!！]\\s?(?i)p([pP]*)?vs(\\s*(?<name>[0-9a-zA-Z\\[\\]\\-_ ]*))?"),
