...
public class GroupMessageEvent extends MessageEvent implements com.now.nowbot.qq.event.GroupMessageEvent {
    ...
    public Group getGroup() {
        return new Group(getBot().getBotID(), event.getGroupId());
    }

    public GroupContact getSender() {
        return new GroupContact(getBot().getBotID(), event.getSender().getUserId(), event.getSender().getNickname(), event.getSender().getRole(), event.getGroupId());
    }
